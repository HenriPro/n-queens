describe('hasVerticalConflicts', function() {
  it('returns true for a verticalConflict', function() {
    var testMatrix = [[0, 1, 0], [0, 1, 0]];
    expect(window.hasVerticalConflicts(testMatrix)).to.equal(true); 
  });
  
  it('returns false for a  non verticalConflict', function() {
    var testMatrix = [[0, 1, 0], [0, 0, 1]];
    expect(window.hasVerticalConflicts(testMatrix)).to.equal(false); 
  });
});
  ...
  describe('findNQueensSolution()', function() {
    ...
    xit('finds a valid solution for n of 0-7', function() {
      ...
      [2, 3].map(function (n) {
        ...
        // console.log('solutionsBoard.get(n)', solutionBoard.get('n'), 'n: ', n);
        expect(solutionBoard.get('n')).to.equal(n);
      });
    });
